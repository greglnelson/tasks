...
suite (`docs/smoketest-f26/FINDINGS.md`, F26-08).
...
## Notes for whoever cuts the student branch

Two things to strip before publishing this to students:

1. **`solutions/` in each chapter folder.** These exist so the exercises are
   provably solvable and are what `VERIFICATION.md` reports on. They are answer
   keys.
2. **`CORRECT_TOOL` in `ch1-setup/checks.ts`.** It is the only fixed answer key in
   the set — every other exercise is graded by measuring the real system — and a
   student reading `checks.ts` will find it. That is not worse than this course's
   existing tests, which spell out every expected value in `functions.test.ts`, but
   it is worth knowing.

Also note: on a branch carrying these stubs, `npm test` is **red by design**, so the
deploy workflow's test step will fail. That is exactly how the existing
`upstream/task-*` branches behave, and it is why this branch is a task branch and
not something to merge into `main` as-is.
